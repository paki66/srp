...
def transform_offense_dim(psql_offense_df, csv_offense_df=None):
    ...
    offense_lookup = (
        psql_offense_df.select(
            ...
            initcap(trim(col("team_name"))).alias("offense_team_name"),
    ...
    if csv_offense_df:
        ...
        offense_df = (
            ...
            .select(
                col("db.offense_id"),
                col("csv.offense_team_name").alias("team_name"),
                col("csv.offense_score").alias("score")
            )
        )

    else:
        offense_df = spark.createDataFrame([], "offense_id long, team_name string, score float")

    window = Window.orderBy("offense_team_name")
    final_df = (
        offense_df
        .dropDuplicates(["team_name", "score"])
        .withColumn("offense_tk", row_number().over(window))
        .select("offense_tk", "team_name", "score")
    )
